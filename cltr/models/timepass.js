...
app.post('/takerReq', async (req, res) => {
  try {
    ...
    if (inventoryItem.quantity >= reqQuantity) {
      // Update the inventory item by subtracting the requested quantity
      inventoryItem.quantity -= reqQuantity;
      await inventoryItem.save();

      // Create a new instance of TakerReqDetails
      ...
      res.status(200).send('Taker request saved successfully. Inventory updated.');
    } else {
      res.status(400).send('Requested quantity is not available in the inventory.');
    }
  } catch (error) {
    console.error(error);
